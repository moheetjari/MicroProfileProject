...
public class ProductManagedBean implements Serializable {
    ...
    ProductJerseyClient jerseyClient ;
    ...
    private Collection<Product> productsbycateogry = new ArrayList<Product>();
    ...
    private String companyName,authorName,categoryName,publisherName,genereName;

    public ProductManagedBean() {
        System.out.println("In ProductManaged Bean ");
        ...
        jerseyClient=new ProductJerseyClient(token);
    }
    ...
    public List<Product> getProducts() {
        String str="productimages/";
//        for (Product product : products) {
//            product.setProductImage(str+product.getProductImage());
...
//            String path = "E:\\RBS\\ReviewBaseSystem\\web\\Admin\\images\\productimages";
...
//            productImage = temp + file.getFileName();
            ...
            //for user
            InputStream input1 = file.getInputStream();
            String path1 = "E:\\RBS\\ReviewBaseSystem\\web\\assets\\productimages";
            ...
            productImage = temp1 + file.getFileName();
            
            Files.copy(input1, new File(path1, productImage).toPath());
    ...
    public String getProduct(String productId) throws IOException {
        ...
        if(product.getAuthorId()!=null)
            authorName = product.getAuthorId().getAuthorName();
        else
            authorName="No Author";
        if(product.getGenreId()!=null)
            genereName = product.getGenreId().getGenreName();
        else
            genereName="No Genre";
        if(product.getPublisherId()!=null)
            publisherName = product.getPublisherId().getPublisherName();
        else
            publisherName="No publisher";
        if(product.getCompanyId()!=null)
            companyName = product.getCompanyId().getCompanyName();
        else
            companyName="No Company";
        categoryName=product.getCategoryId().getCategoryName();
...
//        return "productindex.xhtml?faces-redirect=true";
    ...
    public String addReview() {
        int rid = admin.addReview(productId, new Date(), 11);//write session id instead of 11
